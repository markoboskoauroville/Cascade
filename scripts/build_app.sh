...
if [ -x "$XCBUILD" ]; then
    ...
    BIN="$(swift build -c release --arch arm64 --arch x86_64 --show-bin-path)/Cascade"
else
    echo "==> Building release binary for this machine ($(uname -m))"
    swift build -c release
    BIN="$(swift build -c release --show-bin-path)/Cascade"
fi
...
cp "$BIN" "$APP/Contents/MacOS/Cascade"
cp packaging/Cascade.icns "$APP/Contents/Resources/Cascade.icns"
...
echo "==> Built $APP"
/usr/bin/file "$APP/Contents/MacOS/Cascade"
